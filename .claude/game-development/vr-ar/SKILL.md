...
- **Targets:** Meta Quest (standalone) + PCVR via OpenXR. Hold the **90 FPS floor / 11.11 ms** budget.
- **Per-tier rendering:** Quest drops HQ bloom filtering and motion blur; PCVR keeps them. Don't
  re-enable blur in VR.
- **No camera shake** — it causes judder/sickness. Combat impact comes from `Haptics`,
  `AudioDirector` stingers, and reticle hit-confirm in `CombatFeedbackController`, never shake.
- **Locomotion:** `Player/ContinuousLocomotion.cs` with teleport/snap-turn + comfort vignette;
  respect its posture state guard.
- **Sun glare** is rendered as a head-pinned overlay quad (`Ship/SunNavigation/SunGlare.cs`), not a
  world-space flare — keep it stereo-comfortable.
- World scale is **1 unit = 1 m** — enemy ships, blades, and cockpit must measure true to life.
