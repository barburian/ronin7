...
- **Locations:** source art under `Assets/Ronin7/Art/` (models, materials, textures); prefabs under
  `Assets/Ronin7/Prefabs/`; runtime-loaded assets under `Assets/Ronin7/Resources/`.
- **Generated content:** character/material variants are produced by `Editor/Art/ArtGenerationMenu`.
  The ~288 orphaned materials it leaves behind are regenerable — don't hand-delete them.
- **VR scale:** 1 unit = 1 m, always (see `vr-ar`). Silhouettes must read at in-headset distance.
- **Batching-friendly:** prefer shared materials tinted via `RendererTint` (MPB) over per-object
  material instances, so the renderer keeps batching (see `3d-games`).
