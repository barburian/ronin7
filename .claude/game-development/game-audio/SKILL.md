...
- **Routing:** gameplay audio goes through `Audio/AudioDirector.cs` (combat stingers, ducking) —
  route new SFX/music there rather than scattering raw `AudioSource.Play` calls.
- **Combat juice:** in VR, impact reads through audio + `Haptics` (and reticle hit-confirm), never
  camera shake. Tune stinger timing in-headset.
- **Assets:** audio lives under `Assets/Ronin7/Audio/` (music, SFX, voice). Use 3-5 variations per
  recurring combat sound to avoid repetition.
- **Spatialization:** enemy ship and projectile sounds are 3D-positioned for directional awareness;
  music/UI stay non-diegetic.
