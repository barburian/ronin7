...
namespace Ronin7.Flow
{
    ...
    ///
    /// A mission is one episode: its <see cref="Mission.entryScene"/> is the on-foot scene the hub
    /// loads; the episode then plays through its own internal LandingRequested hops and finishes when
    ...
    public class CampaignDirector : ScriptableObject
    {
        ...
        {
            [Tooltip("Stable episode id, e.g. \"EP02\". For reference/debugging only.")]
            public string id;
        ...
        [Tooltip("Missions in play order (one per episode). Empty entryScene = unfilled scaffold slot.")]
