...
namespace Ronin7.Core
{
    ...
    public static class CampaignStats
    {
        ...
        /// <summary>Records a perfect-parry streak (see <c>Ronin7.Player.ParryFlowController</c>) reaching
        /// <paramref name="reached"/>; BestParryStreak only ever climbs.</summary>
        public static void RecordParryStreak(int reached) => BestParryStreak = NextBestParryStreak(BestParryStreak, reached);
        ...
        /// <summary>Pure Max — the highest parry streak ever reached never decreases. Internal + unit-tested.</summary>
        internal static int NextBestParryStreak(int current, int reached) => Mathf.Max(current, reached);
