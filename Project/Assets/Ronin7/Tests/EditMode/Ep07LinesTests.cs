...
{
    public class Ep07LinesTests
    {
        // Every speaker used in Ep07Lines must appear here AND have an exact-match entry in
        ...
        private static readonly HashSet<string> KnownCast = new HashSet<string>
        {
            ...
            "Dominion Guard 1",
            "Dominion Guard 2",
            "Dominion Interceptor Pilot",
        };

        [Test]
        public void EverySetId_ReturnsNonEmptyArray()
        {
            foreach (string setId in Ep07Lines.SetIds)
            {
                var lines = Ep07Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep07Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep07Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep07Lines.Get("not_a_real_set").Length);
        }
    }
}
