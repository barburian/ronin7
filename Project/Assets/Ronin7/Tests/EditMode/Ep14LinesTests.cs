...
{
    public class Ep14LinesTests
    {
        // Every speaker used in Ep14Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep14Lines.SetIds)
            {
                var lines = Ep14Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep14Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep14Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep14Lines.Get("not_a_real_set").Length);
        }
    }
}
