...
{
    public class Ep04LinesTests
    {
        // Every speaker used in Ep04Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep04Lines.SetIds)
            {
                var lines = Ep04Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep04Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep04Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep04Lines.Get("not_a_real_set").Length);
        }
    }
}
