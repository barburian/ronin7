...
{
    public class Ep29LinesTests
    {
        // Every speaker used in Ep29Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep29Lines.SetIds)
            {
                var lines = Ep29Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep29Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep29Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep29Lines.Get("not_a_real_set").Length);
        }
    }
}
