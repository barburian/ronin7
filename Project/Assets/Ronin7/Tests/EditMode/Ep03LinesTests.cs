...
{
    public class Ep03LinesTests
    {
        // Every speaker used in Ep03Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep03Lines.SetIds)
            {
                var lines = Ep03Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep03Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep03Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep03Lines.Get("not_a_real_set").Length);
        }
    }
}
