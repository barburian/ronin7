...
{
    public class Ep26LinesTests
    {
        // Every speaker used in Ep26Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep26Lines.SetIds)
            {
                var lines = Ep26Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep26Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep26Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep26Lines.Get("not_a_real_set").Length);
        }
    }
}
