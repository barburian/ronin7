...
{
    public class Ep11LinesTests
    {
        // Every speaker used in Ep11Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep11Lines.SetIds)
            {
                var lines = Ep11Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep11Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep11Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep11Lines.Get("not_a_real_set").Length);
        }
    }
}
