...
{
    public class Ep12LinesTests
    {
        // Every speaker used in Ep12Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep12Lines.SetIds)
            {
                var lines = Ep12Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep12Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep12Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep12Lines.Get("not_a_real_set").Length);
        }
    }
}
