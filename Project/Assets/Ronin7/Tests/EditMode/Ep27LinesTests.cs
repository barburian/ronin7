...
{
    public class Ep27LinesTests
    {
        // Every speaker used in Ep27Lines must appear here AND have an exact-match entry in
        ...
        private static readonly HashSet<string> KnownCast = new HashSet<string>
        {
            ...
            "Khall",
            "Khall (Memory)",
            "Dominion Assault Commander",
            "Dominion Weapons Officer",
        };

        [Test]
        public void EverySetId_ReturnsNonEmptyArray()
        {
            foreach (string setId in Ep27Lines.SetIds)
            {
                var lines = Ep27Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep27Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep27Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep27Lines.Get("not_a_real_set").Length);
        }
    }
}
