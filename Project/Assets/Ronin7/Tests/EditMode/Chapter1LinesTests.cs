...
{
    public class Chapter1LinesTests
    {
        // Every speaker used in Chapter1Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Chapter1Lines.SetIds)
            {
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Chapter1Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Chapter1Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Chapter1Lines.Get("not_a_real_set").Length);
        }
    }
}
