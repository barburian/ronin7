...
{
    public class Ep31LinesTests
    {
        // Every speaker used in Ep31Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep31Lines.SetIds)
            {
                var lines = Ep31Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep31Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep31Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep31Lines.Get("not_a_real_set").Length);
        }
    }
}
