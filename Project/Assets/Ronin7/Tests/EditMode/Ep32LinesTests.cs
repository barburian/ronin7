...
{
    public class Ep32LinesTests
    {
        // Every speaker used in Ep32Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep32Lines.SetIds)
            {
                var lines = Ep32Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep32Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep32Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep32Lines.Get("not_a_real_set").Length);
        }
    }
}
