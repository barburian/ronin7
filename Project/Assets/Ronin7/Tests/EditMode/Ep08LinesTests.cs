...
{
    public class Ep08LinesTests
    {
        // Every speaker used in Ep08Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep08Lines.SetIds)
            {
                var lines = Ep08Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep08Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep08Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep08Lines.Get("not_a_real_set").Length);
        }
    }
}
