...
{
    public class Ep28LinesTests
    {
        // Every speaker used in Ep28Lines must appear here AND have an exact-match entry in
        ...
        private static readonly HashSet<string> KnownCast = new HashSet<string>
        {
            ...
            "Khall",
            "Maya Selene",
            "Dominion Scan Agent 1",
            "Dominion Scan Agent 2",
            "Dominion Commander",
        };
        ...
        {
            foreach (string setId in Ep28Lines.SetIds)
            {
                var lines = Ep28Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep28Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep28Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep28Lines.Get("not_a_real_set").Length);
        }
    }
}
