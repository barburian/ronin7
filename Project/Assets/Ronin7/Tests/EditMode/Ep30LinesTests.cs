...
{
    public class Ep30LinesTests
    {
        // Every speaker used in Ep30Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep30Lines.SetIds)
            {
                var lines = Ep30Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep30Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep30Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep30Lines.Get("not_a_real_set").Length);
        }
    }
}
