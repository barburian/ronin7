...
{
    public class Ep16LinesTests
    {
        // Every speaker used in Ep16Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep16Lines.SetIds)
            {
                var lines = Ep16Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep16Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep16Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep16Lines.Get("not_a_real_set").Length);
        }
    }
}
