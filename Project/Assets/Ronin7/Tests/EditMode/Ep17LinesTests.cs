...
{
    public class Ep17LinesTests
    {
        // Every speaker used in Ep17Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep17Lines.SetIds)
            {
                var lines = Ep17Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep17Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep17Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep17Lines.Get("not_a_real_set").Length);
        }
    }
}
