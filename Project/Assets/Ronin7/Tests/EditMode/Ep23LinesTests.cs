...
{
    public class Ep23LinesTests
    {
        // Every speaker used in Ep23Lines must appear here AND have an exact-match entry in
        ...
        private static readonly HashSet<string> KnownCast = new HashSet<string>
        {
            ...
            "Defense Drone 3",
            "Defense Drone 4",
            "Dominion Destroyer Commander",
        };

        [Test]
        public void EverySetId_ReturnsNonEmptyArray()
        {
            foreach (string setId in Ep23Lines.SetIds)
            {
                var lines = Ep23Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep23Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep23Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep23Lines.Get("not_a_real_set").Length);
        }
    }
}
