...
{
    public class Ep24LinesTests
    {
        // Every speaker used in Ep24Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep24Lines.SetIds)
            {
                var lines = Ep24Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep24Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep24Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep24Lines.Get("not_a_real_set").Length);
        }
    }
}
