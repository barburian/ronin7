...
{
    public class Ep18LinesTests
    {
        // Every speaker used in Ep18Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep18Lines.SetIds)
            {
                var lines = Ep18Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep18Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep18Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep18Lines.Get("not_a_real_set").Length);
        }
    }
}
