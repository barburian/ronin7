...
{
    public class Ep25LinesTests
    {
        // Every speaker used in Ep25Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep25Lines.SetIds)
            {
                var lines = Ep25Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep25Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep25Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep25Lines.Get("not_a_real_set").Length);
        }
    }
}
