...
{
    public class Ep21LinesTests
    {
        // Every speaker used in Ep21Lines must appear here AND have an exact-match entry in
        ...
        private static readonly HashSet<string> KnownCast = new HashSet<string>
        {
            ...
            "Lotus Commander",
            "Lotus Guard 1",
            "Dominion Elite Operative 1",
            "Dominion Elite Operative 2",
            "Dominion Elite Operative 3",
        };

        [Test]
        public void EverySetId_ReturnsNonEmptyArray()
        {
            foreach (string setId in Ep21Lines.SetIds)
            {
                var lines = Ep21Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep21Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep21Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep21Lines.Get("not_a_real_set").Length);
        }
    }
}
