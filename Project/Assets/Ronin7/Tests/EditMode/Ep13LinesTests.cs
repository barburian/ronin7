...
{
    public class Ep13LinesTests
    {
        // Every speaker used in Ep13Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep13Lines.SetIds)
            {
                var lines = Ep13Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep13Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep13Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep13Lines.Get("not_a_real_set").Length);
        }
    }
}
