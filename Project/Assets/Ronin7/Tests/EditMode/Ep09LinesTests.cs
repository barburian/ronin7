...
{
    public class Ep09LinesTests
    {
        // Every speaker used in Ep09Lines must appear here AND have an exact-match entry in
        ...
        {
            foreach (string setId in Ep09Lines.SetIds)
            {
                var lines = Ep09Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep09Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep09Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep09Lines.Get("not_a_real_set").Length);
        }
    }
}
