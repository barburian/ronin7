...
{
    public class Ep15LinesTests
    {
        // Every speaker used in Ep15Lines must appear here AND have an exact-match entry in
        ...
        private static readonly HashSet<string> KnownCast = new HashSet<string>
        {
            ...
            "Gilded Maw Operative",
            "Mirs",
            "Emberhand Enforcer Lead",
            "Pale Choir Assassin Lead",
            "Dominion Squad Lead",
            "Syndicate Hunter Lead",
        ...
        {
            foreach (string setId in Ep15Lines.SetIds)
            {
                var lines = Ep15Lines.Get(setId);
        ...
        public void ClipNames_AreUniqueAcrossAllSets()
        {
            ...
            foreach (string setId in Ep15Lines.SetIds)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    string clip = Ep15Lines.ClipName(setId, i, lines[i].speaker);
        ...
        {
            Assert.AreEqual(0, Ep15Lines.Get("not_a_real_set").Length);
        }
    }
}
